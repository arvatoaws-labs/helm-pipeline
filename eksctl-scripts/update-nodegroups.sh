...
# Managed Nodegroups
if [ "$(yq eval '.managedNodeGroups' $CLUSTER_FILE | grep 'name' || echo "nope")" == "nope" ]; then
  ...
  yq eval '.managedNodeGroups' $CLUSTER_FILE | grep name | awk '{ print $3; }' | xargs -i eksctl upgrade nodegroup --cluster=$CLUSTER_NAME --name={}
fi
...
if [ "$(yq eval '.managedNodeGroups' $CLUSTER_FILE | grep 'name' || echo "nope")" == "nope" ]; then
  ...
  NUMBER_OF_NODEGROUPS=$(yq '.nodeGroups | length' $CLUSTER_FILE)
  for (( i=0; i<NUMBER_OF_NODEGROUPS; i++ )); do
    NEW_NAME=$(yq eval $CLUSTER_FILE -j | jq ".nodeGroups[$i].name" | tr -d '"' | sed -e "s/-v[0-9a-f]\+$/-v$COMMIT_ID/g")
